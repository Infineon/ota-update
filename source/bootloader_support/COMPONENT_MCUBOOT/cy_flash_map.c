...
{
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_INFO, "\nMCUBoot flash area:\n");
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_INFO, "      fa_id:  %d\r\n", fa->fa_id);
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_INFO, "  device_id:  %d\r\n", fa->fa_device_id);
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_INFO, "     fa_off:  0x%08lx\r\n", fa->fa_off);
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_INFO, "    fa_size:  0x%08lx\r\n", fa->fa_size);
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_INFO, "  erase_val:  0x%02x\r\n", flash_area_erased_val(fa));
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_INFO, "\r\n");
}
 ...
 void print_all_flash_area_descs(void)
 {
     ...
     cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "\r\nMCUBoot flash areas:\r\n");
...
*/
int flash_area_open(cy_ota_agent_mem_interface_t *flash_handle, uint8_t id, const struct flash_area **fa)
{
    int ret = -1;
    uint32_t i = 0;
...
*/
void flash_area_close(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *fa)
{
    (void)flash_handle;
...
*/
int flash_area_read(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *fa,
                    uint32_t off, void *dst, uint32_t len)
{
    ...
    if((NULL == fa) || (NULL == dst) || (flash_handle->read == NULL) ||
...
#endif
#if defined(PSOC_062_2M) || defined(PSOC_062_1M) || defined(PSOC_062_512K) || defined(PSOC_063_1M) || defined(PSOC_064_2M)
        rc = flash_handle->read(CY_OTA_MEM_TYPE_INTERNAL_FLASH, addr, dst, len);
#endif
    }
#ifdef OTA_USE_EXTERNAL_FLASH
    else if((fa->fa_device_id & FLASH_DEVICE_EXTERNAL_FLAG) == FLASH_DEVICE_EXTERNAL_FLAG)
    {
        rc = flash_handle->read(CY_OTA_MEM_TYPE_EXTERNAL_FLASH, addr, dst, len);
    }
#endif
    else
    {
...
int flash_area_write(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *fa,
                     uint32_t off, const void *src, uint32_t len)
{
    int rc = 0;
    size_t addr = 0;

    /* check if requested offset not less then flash area (fa) start */
    if((NULL == fa) || (NULL == src) || (flash_handle->write == NULL) ||
       (off > fa->fa_size || len > fa->fa_size || off + len > fa->fa_size))
    {
        return BOOT_EBADARGS;
    }
...
#endif
#if defined(PSOC_062_2M) || defined(PSOC_062_1M) || defined(PSOC_062_512K) || defined(PSOC_063_1M) || defined(PSOC_064_2M)
        rc = flash_handle->write(CY_OTA_MEM_TYPE_INTERNAL_FLASH, addr, (void *)src, len);
#endif
    }
#ifdef OTA_USE_EXTERNAL_FLASH
    else if((fa->fa_device_id & FLASH_DEVICE_EXTERNAL_FLAG) == FLASH_DEVICE_EXTERNAL_FLAG)
    {
        rc = flash_handle->write(CY_OTA_MEM_TYPE_EXTERNAL_FLASH, addr, (void *)src, len);
    }
#endif
    else
    {
...
int flash_area_erase(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *fa,
                     uint32_t off, uint32_t len)
{
    ...
    /* check if requested offset not less then flash area (fa) start */
    if((NULL == fa) || (flash_handle->erase == NULL))
    {
        return BOOT_EBADARGS;
    }
...
#endif
#if defined(PSOC_062_2M) || defined(PSOC_062_1M) || defined(PSOC_062_512K) || defined(PSOC_063_1M) || defined(PSOC_064_2M)
        rc = flash_handle->erase(CY_OTA_MEM_TYPE_INTERNAL_FLASH, addr, len);
#endif
    }
#ifdef OTA_USE_EXTERNAL_FLASH
    else if((fa->fa_device_id & FLASH_DEVICE_EXTERNAL_FLAG) == FLASH_DEVICE_EXTERNAL_FLAG)
    {
        rc = flash_handle->erase(CY_OTA_MEM_TYPE_EXTERNAL_FLASH, addr, len);
    }
...
size_t flash_area_align(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *fa)
{
    ...
    if(NULL != fa)
    {
        ...
        {
            if(flash_handle->get_prog_size != NULL)
            {
                rc = flash_handle->get_prog_size(CY_OTA_MEM_TYPE_EXTERNAL_FLASH, 0);
            }
        }
#endif /* OTA_USE_EXTERNAL_FLASH */
        else
 ...
 */
int boot_write_trailer(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *fap,
                       uint32_t off, const uint8_t *inbuf, uint8_t inlen)
{
    ...
    align = flash_area_align(flash_handle, fap);
    if(align == 0u)
    {
        return BOOT_EFLASH;
    }
    ...
    {
        cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%d:%s() align 0x%x > 0x%x BOOT_MAX_ALIGN , ret -1\n", __LINE__, __func__, align, BOOT_MAX_ALIGN);
        return -1;
    }
    ...
    rc = flash_area_write(flash_handle, fap, off, buf, align);
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%d:%s() flash_area_write(fap, 0x%lx, %p, 0x%lx) , ret %d\n", __LINE__, __func__, off, buf, align, rc);
    if(rc != 0)
    {
        return BOOT_EFLASH;
    }
...
int boot_write_magic(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *fap)
{
    ...
    rc = flash_area_write(flash_handle, fap, off, magic, BOOT_MAGIC_SZ);
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s() fap->off 0x%lx off 0x%lx rc %d\n", __func__, fap->fa_off, off, rc);
    if(rc != 0)
    {
        return BOOT_EFLASH;
    }
...
int boot_write_image_ok(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *fap)
{
    uint32_t off;
    uint8_t  buff[2];
    int rc;
    ...
    rc = flash_area_write(flash_handle, fap, off, buff, 1);
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s() fap->off 0x%lx off 0x%lx rc %d\n", __func__, fap->fa_off, off, rc);
    if(rc != 0)
    {
        return BOOT_EFLASH;
 ...
 */
int boot_write_swap_info(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *fap,
                         uint8_t swap_type, uint8_t image_num)
{
    uint32_t off;
    uint8_t swap_info;
    int     rc;
    ...
    rc = boot_write_trailer(flash_handle, fap, off, (const uint8_t *) &swap_info, 1);
    cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s() fap->off 0x%lx off 0x%lx rc %d\n", __func__, fap->fa_off, off, rc);
    if(rc != 0)
    {
        return BOOT_EFLASH;
...
int flash_area_boot_set_pending(cy_ota_agent_mem_interface_t *flash_handle, int image, int permanent)
{
    ...
    rc = flash_area_open(flash_handle, FLASH_AREA_IMAGE_SECONDARY(image), &fap);
    if(rc == 0)
    {
        rc = boot_write_magic(flash_handle, fap);
        cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s() boot_write_magic(fap) returned %d\n", __func__, rc);
        ...
        if( (fap->fa_device_id & FLASH_DEVICE_EXTERNAL_FLAG) == FLASH_DEVICE_EXTERNAL_FLAG)
        {
            ...
            if(rc == 0 && permanent)
            {
                rc = boot_write_image_ok(flash_handle, fap);
                cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s() boot_write_image_ok(fap) returned %d\n", __func__, rc);
            }
            ...
            if(rc == 0)
            {
                ...
                rc = boot_write_swap_info(flash_handle, fap, swap_type, 0);
                cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s() boot_write_swap_info(fap) returned %d\n", __func__, rc);
            }
        }

        flash_area_close(flash_handle, fap);
    }
...
int flash_area_boot_set_confirmed(cy_ota_agent_mem_interface_t *flash_handle)
{
    ...
    rc = flash_area_open(flash_handle, FLASH_AREA_IMAGE_PRIMARY(0), &fap);
    if(rc == 0)
    {
        rc = boot_write_magic(flash_handle, fap);
        cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s() boot_write_magic() returned %d\n", __func__, rc);
        if(rc == 0)
        {
            rc = boot_write_image_ok(flash_handle, fap);
            cy_ota_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "boot_write_image_ok() returned %d\n", rc);
        }
        flash_area_close(flash_handle, fap);
    }
