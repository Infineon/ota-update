...
int flash_area_open(cy_ota_agent_mem_interface_t *flash_handle, uint8_t id, const struct flash_area **);
void flash_area_close(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *);
/*< Reads `len` bytes of flash memory at `off` to the buffer at `dst` */
int flash_area_read(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *, uint32_t off, void *dst,
                     uint32_t len);
/*< Writes `len` bytes of flash memory at `off` from the buffer at `src` */
int flash_area_write(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *, uint32_t off,
                     const void *src, uint32_t len);
/*< Erases `len` bytes of flash memory at `off` */
int flash_area_erase(cy_ota_agent_mem_interface_t *flash_handle, const struct flash_area *, uint32_t off, uint32_t len);

/* writes MAGIC, OK and swap_type */
int flash_area_boot_set_pending(cy_ota_agent_mem_interface_t *flash_handle, int image, int permanent);

/* writes MAGIC and OK */
int flash_area_boot_set_confirmed(cy_ota_agent_mem_interface_t *flash_handle);
