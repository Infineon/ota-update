			    ...
			    const void *data, size_t len)
{
	...
	if (rc != 0 )
	{
//	    printf("psoc6_flash_write_hal(0x%08x, 0x%08x, %p) FAILED rc:%d\n", data, addr, len, rc);
	}
...
//    printf("%d:%s() 0x%lx + 0x%lx = 0x%lx, CY_FLASH_BASE 0x%lx CY_FLASH_SIZE 0x%lx cond1 %d \n", __LINE__, __func__, eeOffset, len, eeOffset+len, CY_FLASH_BASE, CY_FLASH_SIZE, cond1);
    ...
    if(cond1)
    {
        ...
        while((srcIndex < len) && (rc == CY_FLASH_DRV_SUCCESS))
        {
            ...
            if(rowsNotEqual != 0u)
            {
                ...
                if (rc != 0)
                {
//                    printf("%d:%s() called Cy_Flash_WriteRow(%p, %p) rc:%d\n", __LINE__, __func__, (uint8_t*)((rowId * CY_FLASH_SIZEOF_ROW) + CY_FLASH_BASE), (uint8_t*)writeBuffer, rc);
                }
            }
